...
def _extract_event_text(event: Dict[str, Any], part: Dict[str, Any]) -> tuple[Optional[str], str]:
    ...
    return text_type, text if isinstance(text, str) else ""
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        if returncode != 0:
            raise HTTPException(status_code=500, detail=f"chat failed: {stderr}")
        ...
        if not response_text.strip() and not reasoning_text.strip() and not tools:
            ...
            if fallback_code != 0:
                raise HTTPException(status_code=500, detail=f"chat fallback failed: {fallback_stderr}")
